...
- **`distcc/distcc` (upstream) is read-only, always, no exceptions.** This clone has an `upstream` remote pointing at it — never push, open/comment/edit a PR or issue, or take any write action against it; always pass `--repo wiki-mod/distcc-ng` explicitly on every `gh` subcommand that resolves a repository ambiently. `gh api` is a documented exception (it has no `--repo` flag): a literal endpoint path already unambiguously names the target repo, an `{owner}`/`{repo}` endpoint-path placeholder resolves via `GH_REPO`, and an `-F`/`--field` placeholder (e.g. `-F owner='{owner}'`) resolves only from the current directory's repo with no `GH_REPO` override — use literal `owner`/`repo` strings there instead. See `AGENTS.md`'s "What Not To Do" and rules 18/62-64 for the full rule and the incidents that made each part necessary.
