...
- **Read anywhere, write only here.** Reading/searching any repo (including `distcc/distcc` upstream) is fine, no permission needed. Writing — opening/commenting/editing/closing an issue or PR, pushing, merging — anywhere other than `wiki-mod/distcc-ng` requires the maintainer's explicit, per-action authorization first; never assume a past approval carries over. This clone has an `upstream` remote pointing at `distcc/distcc` — always pass `--repo wiki-mod/distcc-ng` explicitly on every `gh` command as the mechanical safeguard. See `AGENTS.md`'s "What Not To Do" for the full rule and the incident that made it necessary.
