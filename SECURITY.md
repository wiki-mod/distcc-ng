...
- **GitHub Actions** (`.github/workflows/**`): the one ecosystem with both a manifest (each workflow's `uses:` lines) and real automated tooling. **OSV-Scanner** (run by `.github/scripts/ci.sh`, via `security.yml`'s `osv-scan` job) gates every pull request and push to `current_dev`/`master` against OSV.dev's advisory database, in addition to Dependabot's periodic update PRs (`.github/dependabot.yml`).
...
- **GitHub Actions secrets are the only secret material this project uses** (e.g. registry/publish tokens consumed by `.github/workflows/*.yml`). No API keys, passwords, or private keys are checked into source, workflows, or docs (see `AGENTS.md` rule `[AG-SEC-001]`).
- **Least-privilege workflow permissions**: workflow files default to `contents: read` at the top level, with each job declaring only the specific additional write scope it actually needs (e.g. `packages: write` only on the job that pushes to GHCR) — see `.github/workflows/release.yml` for the current example, tightened repo-wide in #308.
...
(`--owner wiki-mod` also works if you want to trust any repository under the `wiki-mod` organization rather than this specific one.) A successful verification cryptographically confirms the artifact's checksum matches an attestation signed by this repository's own `release.yml` workflow run — i.e. it was built here, by this project's CI, and not modified since. See `gh attestation verify --help` for additional flags (e.g. pinning a specific signer workflow path).
