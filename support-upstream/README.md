...
| 42 | none filed | #284 (purge behavior only) | [`rpm.spec`'s Debian branch removes the `distcc` service user/group on purge, unlike Debian's own real package](issue-packaging-user-lifecycle-inconsistency.md) | Purge behavior fixed by #284. Still present in upstream's live source: a separate, unfixed home-directory inconsistency (RH branch creates a home dir via `-m`, Debian branch uses `--home /` instead of a genuinely inert path) — documented in the same entry, out of scope for this change |
