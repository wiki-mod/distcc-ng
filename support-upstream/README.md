...
| 45 | [#264](https://github.com/wiki-mod/distcc-ng/issues/264) | not yet merged | [No full bidirectional native-compatibility distributed-build E2E test](issue-264-bidirectional-e2e.md) | Not a bug — upstream's CI (`c-build.yml`) only ever tests a same-build `distccd`/`distcc` pair on `localhost`; no two-container/two-host job, no independently-built native counterpart, no real third-party project distributed across a network boundary |
