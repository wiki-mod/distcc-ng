...
if [ -z "${GH_TOKEN:-}" ]; then
    ...
    repo_name="${repo#*/}"
    # shellcheck disable=SC2016 -- single quotes are deliberate here: this
    # is a Python heredoc-style string with shell variables interpolated
    # only at the specific '"$var"' break-outs below, not throughout.
    query=$(python3 -c '
import json
q = """
