...
| `fs-jail` | `off` \| `optional` \| `required` | `off` | Linux mount-namespace filesystem jail for server-side (pump-mode) compile jobs (issue #289), separate from the seccomp sandbox above. `off` (default): no jail, behaviour unchanged. `optional`: enter the jail, but a setup failure before the point of no return logs a warning and proceeds unjailed (fail-open). `required`: any setup failure refuses the compile (fail-closed). A no-op on non-Linux builds. See `doc/fs-jail-security-checklist.md` for the full design and scope. |
