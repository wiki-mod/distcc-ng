...
| `fail-open` | bool | `true` | `false` (fail-closed) means a sandbox-install failure refuses the compile instead of letting it proceed unsandboxed. Also applies to a build compiled **without** libseccomp at all (`--without-seccomp` or non-Linux): with `fail-open = false`, such a build refuses every remote compile rather than always running them unsandboxed. |
