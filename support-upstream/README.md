...
| 59 | [#493](https://github.com/wiki-mod/distcc-ng/issues/493) | [#495](https://github.com/wiki-mod/distcc-ng/pull/495) | [`c-build.yml`'s `ConorMacBride/install-package` step has no retry, so a hung package-manager mirror burns the whole job timeout](issue-493-ci-mirror-hang-retry.md) | Still present in upstream's live source; upstream's own merged #565 shortened `timeout-minutes` after a similar hang but added no retry |
