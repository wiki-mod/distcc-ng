...
| 40 | none filed | this change | [`rpm.spec`'s Red Hat and Debian branches disagree on the `distcc` service user's home-directory/purge-deletion lifecycle](issue-packaging-user-lifecycle-inconsistency.md) | Still present in upstream's live source; RH branch creates a home dir (`-m`), Debian branch doesn't, and Debian branch deletes the user on purge while Debian's own real package never does |
