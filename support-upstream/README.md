...
| 5 | [#73](https://github.com/wiki-mod/distcc-ng/issues/73) | [#175](https://github.com/wiki-mod/distcc-ng/pull/175) / [#245](https://github.com/wiki-mod/distcc-ng/pull/245) / [#280](https://github.com/wiki-mod/distcc-ng/pull/280) | [-march=native/-mtune=native hard-fail to local — upstream has an open, unmerged fix with real bugs of its own](issue-073-march-native-resolve.md) | Upstream `master` still hard-fails; upstream's own open PRs #350/#384 have real unfixed bugs this fork's port found |
...
| 37 | [#264](https://github.com/wiki-mod/distcc-ng/issues/264) | [#273](https://github.com/wiki-mod/distcc-ng/pull/273) | [Dedicated verification/debug container — not a live upstream bug, a fork-only tooling addition](issue-264-verification-container.md) | Not a bug — upstream's `docker/` tree is a narrow old-compiler-compatibility harness, no general verification/debug image equivalent |
| 38 | [#76](https://github.com/wiki-mod/distcc-ng/issues/76) | [#276](https://github.com/wiki-mod/distcc-ng/pull/276) | [`tweak_arguments_for_server()` never rewrites `-f*-prefix-map=` build-path prefixes](issue-076-serve-prefix-map.md) | Still present in upstream's live source; upstream's own open PR #459 (opened 2022-04-29) proposes the same fix, never merged |
| 39 | [#229](https://github.com/wiki-mod/distcc-ng/issues/229) | [#277](https://github.com/wiki-mod/distcc-ng/pull/277) | [`setup.py`'s environment `CFLAGS` is appended after, not instead of, Python's own sysconfig `CFLAGS`/`OPT`/`LDSHARED`](issue-229-setup-py-cflags-append.md) | Identical mechanism present in upstream's live source, but not visibly triggered under upstream's own default (unmodified `-O2`) build; would reproduce the same double-flag symptom under any custom `CFLAGS` override |
| 40 | [#78](https://github.com/wiki-mod/distcc-ng/issues/78) | [#281](https://github.com/wiki-mod/distcc-ng/pull/281) | [`dcc_add_clang_target()`/`dcc_gcc_rewrite_fqn()` match argv[0] literally, so cross-compile auto-rewrite never fires when the compiler is invoked by full path](issue-078-cross-compile-fullpath-basename.md) | Still present in upstream's live source; upstream's own open PR #491 (opened, unmerged) proposes the same fix, independently re-verified here |
| 41 | [#77](https://github.com/wiki-mod/distcc-ng/issues/77) | [#279](https://github.com/wiki-mod/distcc-ng/pull/279) | [`distccd` never sets its Linux autogroup niceness, so `nice(2)`'s effect can be silently defeated by `setsid()`](issue-077-autogroup-niceness.md) | Still present in upstream's live source; upstream's own open PR #468 proposes the same fix, never merged |
| 42 | [#292](https://github.com/wiki-mod/distcc-ng/issues/292) | [#293](https://github.com/wiki-mod/distcc-ng/pull/293) | [distccd follows a symlink at an intermediate `NAME` component when materializing a later multi-file entry (compound path-traversal write-escape)](issue-292-name-symlink-traversal.md) | Still present in upstream's live source (`open()`/`mkdir()`/`symlink()` with no `O_NOFOLLOW`, both `FIXME`s intact, commit `8d569d1`), unreported |
| 42 | none filed | [#284](https://github.com/wiki-mod/distcc-ng/pull/284) | [`rpm.spec` disagrees with itself, and with Debian's own real package, on the `distcc` service user's lifecycle](issue-packaging-user-lifecycle-inconsistency.md) | Still present in upstream's live source: Debian branch removes the user/group on purge (real Debian package never does), and both branches disagree on the home directory (RH creates one via `-m`, Debian uses a nonsensical `--home /`) — neither matches Debian's own real `--home /nonexistent` convention |
| 43 | none filed (found during #292) | [#294](https://github.com/wiki-mod/distcc-ng/pull/294) | [`PathSafety_Case` defined but never registered in `test/testdistcc.py`'s `tests` list, silently never run under `make check`](issue-pathsafety-test-registration.md) | Not a bug — the tested functions and the test class are both fork-only additions (issues #93/#100/#95), upstream's `testdistcc.py` has no equivalent to compare against |
| 44 | [#181](https://github.com/wiki-mod/distcc-ng/issues/181) | [#296](https://github.com/wiki-mod/distcc-ng/pull/296) | [`distcc-ng-pump`: establish an actively-maintained pump-mode container build variant, separately published](issue-181-pump-container-variant.md) | Not a bug — upstream's `docker/` tree is a narrow compiler-compatibility harness, no general release/runtime image (pump-capable or otherwise) to compare against |
| 45 | [#101](https://github.com/wiki-mod/distcc-ng/issues/101) | [#301](https://github.com/wiki-mod/distcc-ng/pull/301) | [Zstd + server-side cpp (pump mode) combination has no protocol version](issue-101-zstd-pump-protover.md) | Not applicable — zstd itself is a distcc-ng-only feature (see row 26); upstream has no protocol version above `DCC_VER_3` at all, so this combination cannot be a gap upstream either |
| 46 | [#264](https://github.com/wiki-mod/distcc-ng/issues/264) | [#302](https://github.com/wiki-mod/distcc-ng/pull/302) | [No full bidirectional native-compatibility distributed-build E2E test](issue-264-bidirectional-e2e.md) | Not a bug — upstream's CI (`c-build.yml`) only ever tests a same-build `distccd`/`distcc` pair on `localhost`; no two-container/two-host job, no independently-built native counterpart, no real third-party project distributed across a network boundary |
| 47 | none filed (surfaced live via #263) | [#348](https://github.com/wiki-mod/distcc-ng/pull/348) | [Spurious "getline failed" warning on plain EOF in `dcc_check_unsupported_directives()`](issue-263-getline-eof-errno-warning.md) | Still present in upstream's live source: no `errno` reset before the `getline()` loop and no `ferror()` check, so a stale `errno` from an unrelated earlier syscall can be misreported as a read failure on a clean EOF |
| 48 | none filed (surfaced live via #229/#266) | [#352](https://github.com/wiki-mod/distcc-ng/pull/352) | [NULL-source, zero-length `memcpy()` UB on the first call to `dcc_add_cleanup()`](issue-266-cleanup-null-memcpy.md) | Still present in upstream's live source: no guard against `cleanups_size == 0` before the `memcpy()`, so UBSan flags it on the first cleanup registration in any real compile or daemon session |
| 49 | [#398](https://github.com/wiki-mod/distcc-ng/issues/398) | [#400](https://github.com/wiki-mod/distcc-ng/pull/400) | [`pump.in`'s `ShutDown()` never sends SIGTERM when `ps -p` isn't supported (e.g. BusyBox)](issue-398-pump-busybox-ps.md) | Still present in upstream's live source: `ShutDown()`'s own `ps -p "$include_server_pid"` check has the identical unconditional dependency, so the include server is never sent SIGTERM on any BusyBox-based system either |
| 50 | [#287](https://github.com/wiki-mod/distcc-ng/issues/287) | [#406](https://github.com/wiki-mod/distcc-ng/pull/406) | [`dcc_execvp()`'s directory-qualified-path failure silently retries with a bare-basename `$PATH` search](issue-287-execvp-path-fallback.md) | Still present in upstream's live source: the identical second `execvp()` basename-PATH-search fallback, unreported |
| 51 | [#379](https://github.com/wiki-mod/distcc-ng/issues/379) | [#415](https://github.com/wiki-mod/distcc-ng/pull/415) | [Test daemon's `--lifetime` alarm can kill it mid-test on a slow/loaded runner, racing normal teardown](issue-379-lifetime-alarm-races-teardown.md) | Still present in upstream's live source (identical `daemon_lifetime()` default/overrides), unreported |
| 52 | none filed | [#416](https://github.com/wiki-mod/distcc-ng/pull/416) | [`--include=` (GCC/Clang's combined-form force-include flag) never rewritten for the server in pump mode](issue-416-include-equals-server-rewrite.md) | Still present in upstream's live source (both `serve.c` and `include_server/parse_command.py`), unreported |
| 53 | none filed (relates to #275) | [#417](https://github.com/wiki-mod/distcc-ng/pull/417) | [`runcmd_background()`'s shell-exec-vs-fork ambiguity is a live, general hazard beyond the one case upstream already fixed](issue-417-client-disconnect-kills-child.md) | Not a bug in shipped functionality — a test-harness hazard upstream partially fixed (PR #548) for one case, not the general helper; the underlying `"children are killed off"` TODO is also still present, untested, in upstream's own source |
| 54 | none filed | [#418](https://github.com/wiki-mod/distcc-ng/pull/418) | [`--imacros=` (GCC/Clang's combined form of `-imacros`) never rewritten for the server in pump mode](issue-418-imacros-equals-server-rewrite.md) | Still present in upstream's live source (both `serve.c` and `include_server/parse_command.py`), unreported |
| 55 | none filed | [#419](https://github.com/wiki-mod/distcc-ng/pull/419) | [`-isysroot`/`--sysroot=` never rewritten for the server in pump mode](issue-419-sysroot-server-rewrite.md) | Still present in upstream's live source (no entry at all, either form), unreported |
| 56 | [#442](https://github.com/wiki-mod/distcc-ng/issues/442) | [#450](https://github.com/wiki-mod/distcc-ng/pull/450) | [Pump mode's include server can't see through a `ccache` wrapper (`compiler = args[0]`)](issue-442-pump-ccache-wrapper.md) | Still present in upstream's live source, unreported (related discussion in open distcc/distcc#230, distinct problem) |
| 57 | [#460](https://github.com/wiki-mod/distcc-ng/issues/460) | [#466](https://github.com/wiki-mod/distcc-ng/pull/466) | [Test-fixture file writes not explicitly closed before the next test step](issue-testdistcc-unclosed-write-before-subprocess-read.md) | Still present in upstream's live source at all seven cited call sites (across six methods/classes), unreported |
| 58 | [#398](https://github.com/wiki-mod/distcc-ng/issues/398) | [#487](https://github.com/wiki-mod/distcc-ng/pull/487) | [`dcc_fix_debug_info()` silently fails to rewrite compressed ELF debug sections](issue-398-compressed-debug-info.md) | Still present in upstream's live source (byte-identical `replace_string()`/`update_section()` design), unreported |
| 59 | [#398](https://github.com/wiki-mod/distcc-ng/issues/398) | [#527](https://github.com/wiki-mod/distcc-ng/pull/527) | [Split DWARF (`-gsplit-dwarf`) has no external-`.dwo` transport for server-side cpp (pump mode)](issue-398-split-dwarf-pump.md) | Still present in upstream's live source (only protocols 1/2/3; no `DDWO` slot in `serve.c`/`clirpc.c`, no `.dwo` handling at all), unreported |
| 58 | [#398](https://github.com/wiki-mod/distcc-ng/issues/398) | [#487](https://github.com/wiki-mod/distcc-ng/pull/487), [#526](https://github.com/wiki-mod/distcc-ng/pull/526) | [`dcc_fix_debug_info()` silently fails to rewrite compressed ELF debug sections](issue-398-compressed-debug-info.md) | Still present in upstream's live source for both `SHF_COMPRESSED` and legacy GNU `.zdebug_*` (byte-identical `replace_string()`/`update_section()` design; no `.zdebug_*` names or `elf_compress*` anywhere), unreported |
