...
| `c-build.yml` | `push` (`current_dev`/`master`), `pull_request` (unfiltered), `workflow_dispatch`, `schedule 03:00 daily` | Core build+test gate: `make_check` (macOS+Linux matrix), `popt_fallback_build`, `popt_vendor_check`, `distributed_e2e` (builds `test/e2e/` fresh) | build-provenance attestations |
