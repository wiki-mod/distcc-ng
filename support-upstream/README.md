...
| 37 | [#229](https://github.com/wiki-mod/distcc-ng/issues/229) | [#277](https://github.com/wiki-mod/distcc-ng/pull/277) | [`setup.py`'s environment `CFLAGS` is appended after, not instead of, Python's own sysconfig `CFLAGS`/`OPT`/`LDSHARED`](issue-229-setup-py-cflags-append.md) | Identical mechanism present in upstream's live source, but not visibly triggered under upstream's own default (unmodified `-O2`) build; would reproduce the same double-flag symptom under any custom `CFLAGS` override |
