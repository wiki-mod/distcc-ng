...
| 49 | none filed (surfaced live via #398) | [#400](https://github.com/wiki-mod/distcc-ng/pull/400) | [`pump.in`'s `ShutDown()` never fires when `ps -p` isn't supported (e.g. BusyBox)](issue-398-pump-busybox-ps.md) | Still present in upstream's live source: `ShutDown()`'s own `ps -p "$include_server_pid"` check has the identical unconditional dependency, so the include server is never sent SIGTERM on any BusyBox-based system either |
