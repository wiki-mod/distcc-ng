...
| 39 | [#78](https://github.com/wiki-mod/distcc-ng/issues/78) | [#281](https://github.com/wiki-mod/distcc-ng/pull/281) | [`dcc_add_clang_target()`/`dcc_gcc_rewrite_fqn()` match argv[0] literally, so cross-compile auto-rewrite never fires when the compiler is invoked by full path](issue-078-cross-compile-fullpath-basename.md) | Still present in upstream's live source; upstream's own open PR #491 (opened, unmerged) proposes the same fix, independently re-verified here |
